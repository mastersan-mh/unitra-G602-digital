Руководство пользователя устройства электропроигрывающего Unitra-G602(digital)
========
...
| `[33]`           | Включение базовой скорости 33 1/3 об/мин | Нет       |
| `[АВТОСТОП]`     | Нет                                      | Нет       |
...
3. Прочитать коэффициенты ПИД-регулятора: Kp, Ki, Kd
    ...
    * Ответ:
    ...
    `<resv[0]>`, `<resv[1]>` - `<Kp>`, пропорциональный коэффициент в формате signed-fixed-point-16.16.

    `<resv[2]>`, `<resv[3]>` - `<Ki>`, интегральный коэффициент в формате signed-fixed-point-16.16.

    `<resv[4]>`, `<resv[5]>` - `<Kd>`, дифференциальный коэффициент в формате signed-fixed-point-16.16
...
4. Задать коэффициенты ПИД-регулятора: Kp, Ki, Kd
    ...
    * Запрос:
    ...
    `<argv[0]>`, `<argv[1]>` - `<Kp>`, пропорциональный коэффициент в формате signed-fixed-point-16.16.

    `<argv[2]>`, `<argv[3]>` - `<Ki>`, интегральный коэффициент в формате signed-fixed-point-16.16.

    `<argv[4]>`, `<argv[5]>` - `<Kd>`, дифференциальный коэффициент в формате signed-fixed-point-16.16
...
5. Прочитать уставку скорости вращения стола
    ...
    * Ответ:
    ...
    `<resv[0]>`, `<resv[1]>` - Скорость вращения, число в диапазоне  `[0; 51 * <imp>]` импульсов/мин в формате signed-fixed-point-16.16.
...
6. Задать уставку скорости вращения стола
    ...
    * Запрос:
    ...
    `<argv[0]>`, `<argv[1]>` - Скорость вращения, число в диапазоне  `[ 0; 51 * <imp> ]` импульсов/мин в формате signed-fixed-point-16.16.
...
2. Событие SP/PV
    ...
    `<resv[0]>`, `<resv[1]>` - Время от запуска микроконтроллера, старшая и младшая часть, мс.

    `<resv[2]>`, `<resv[3]>` - Setpoint (Уставка), cкорость вращения, число в диапазоне  `[0; 51 * <imp>]` импульсов/мин в формате signed-fixed-point-16.16.

    `<resv[4]>`, `<resv[5]>` - Process Variable (Переменная процесса), cкорость вращения, число в диапазоне `[0; 51 * <imp>]` импульсов/мин в формате signed-fixed-point-16.16.

    `<resv[6]>`, `<resv[7]>` - Переменная управления в формате signed-fixed-point-16.16
